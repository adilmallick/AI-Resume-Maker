...
**LLM Configurations:**
You can use either Local AI (Ollama) or Cloud AI (Groq API):
1. **(Option A) Ollama:** Install [Ollama](https://ollama.com/download) and pull local models:
   ```bash
   ollama pull llama3.1:8b       # Default
   ollama pull gemma2:9b         # Alternative
   ollama pull nomic-embed-text
   ```
2. **(Option B) Groq:** Get a free API key from [console.groq.com](https://console.groq.com/keys) and set `GROQ_API_KEY=gsk_...` in your `.env` file. Then set `LLM_PROVIDER=groq`.
**PostgreSQL (Docker):**
...
# Setup Environment Variables (Optional for Groq)
...
To deploy this exactly as built without spending money:
- **Next.js Frontend:** Host on **Vercel** (Free Hobby Tier).
- **FastAPI / PDF Generator:** Host on **Render** as a Docker Web Service. Render's free tier spins down after 15 mins but works perfectly. Ensure the `Dockerfile` installs `texlive-latex-base` so `pdflatex` can compile.
- **Database:** Host PostgreSQL on **Neon.tech** or **Supabase** (Free limits easily cover personal resumes).
- **LLM / AI Engine:** Use the **Groq API** wrapper (`LLM_PROVIDER=groq`). Groq's high-speed inference replaces heavy local Ollama requirements for zero cost.
