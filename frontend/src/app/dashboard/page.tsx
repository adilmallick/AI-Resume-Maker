    ...
    <>
      <div className="container" style={{ marginTop: '40px' }}>
        ...
        <div className="glass-panel" style={{ marginBottom: "30px", background: "var(--bg-surface)", border: "1px solid var(--accent-glow)", padding: 0 }}>
          ...
          <div className="" style={{ padding: '30px', background: 'rgba(108, 93, 211, 0.05)', borderColor: 'rgba(108, 93, 211, 0.2)' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '30px', overflowX: 'auto', paddingBottom: '10px', borderBottom: '1px solid var(--border-color)' }}>
          {['profile', 'experience', 'projects', 'education', 'skills'].map(tab => (
            <button
              key={tab}
              type="button"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "32px", alignItems: "stretch", width: "100%" }}>
          ...
          <div style={{ marginTop: '20px', width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              {!isEditingProfile && <button className="btn btn-secondary" style={{ padding: '8px 16px', fontSize: '0.9rem' }} onClick={() => setIsEditingProfile(true)}>Edit Profile</button>}
            </div>
          ...
          <div style={{ marginTop: '10px', width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              {!isAddingSocial && <button className="btn btn-secondary" style={{ padding: '8px 16px', fontSize: '0.9rem' }} onClick={() => setIsAddingSocial(true)}>+ Add Social</button>}
            </div>

            <Modal isOpen={isAddingSocial} onClose={() => { setIsAddingSocial(false); setEditingSocialId(null); setSocialForm({ platform_name: '', url: '', display_text: '' }); }} title={editingSocialId ? 'Edit Social' : 'Add Social'}>
              <form onSubmit={handleSaveSocial} style={{ marginBottom: '15px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button className="btn btn-primary" type="submit" style={{ flex: 1, padding: '10px' }} disabled={isSavingSocial}>{isSavingSocial ? "Saving..." : "Save"}</button>
                  <button className="btn btn-secondary" type="button" onClick={() => { setIsAddingSocial(false); setEditingSocialId(null); setSocialForm({ platform_name: '', url: '', display_text: '' }); }} style={{ flex: 1, padding: '10px' }} disabled={isSavingSocial}>Cancel</button>
                </div>
              </form>
            </Modal>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              {socials.length === 0 && !isAddingSocial ? <div className="glass-card" style={{ gridColumn: '1 / -1', textAlign: 'center', color: 'var(--text-muted)', padding: '40px' }}>No socials listed.</div> : null}
              {socials.map(soc => (
                <div key={soc.id} className="glass-card" style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      <button className="btn btn-secondary" style={{ padding: '2px 8px', fontSize: '0.75rem' }} onClick={() => handleEditSocial(soc)}>Edit</button>
                      <button className="btn btn-secondary" style={{ padding: '2px 8px', fontSize: '0.75rem', color: 'var(--error)', borderColor: 'var(--error)' }} onClick={() => handleDeleteSocial(soc.id)}>Delete</button>
                    </div>
                  </div>
                  <a href={soc.url} target="_blank" rel="noreferrer" style={{ fontSize: '0.9rem', color: 'var(--accent-light)', textDecoration: 'underline', wordBreak: 'break-all' }}>{soc.display_text || soc.url}</a>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="dashboard-list" style={{ width: "100%" }}>
            ...
            {/* Experiences */}
            <div style={{ marginTop: '20px', display: activeTab === 'experience' ? 'block' : 'none', width: '100%' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <Modal isOpen={isAddingExp} onClose={() => { setIsAddingExp(false); setEditingExpId(null); setExpForm({ company_name: '', job_title: '', location: '', start_date: '', end_date: '', raw_description: '' }); }} title={editingExpId ? 'Edit Experience' : 'Add New Experience'}>
                <form onSubmit={handleSaveExp} style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: activeTab === 'projects' ? 'block' : 'none', width: '100%' }}>
              ...
              <Modal isOpen={isAddingProj} onClose={() => { setIsAddingProj(false); setEditingProjId(null); setProjForm({ title: '', role: '', tech_stack: '', repository_url: '', live_demo_url: '', start_date: '', end_date: '', raw_description: '' }); }} title={editingProjId ? 'Edit Project' : 'Add New Project'}>
                <form onSubmit={handleSaveProj} style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
            ...
            <div style={{ display: activeTab === 'education' ? 'block' : 'none', width: '100%' }}>
              ...
              <Modal isOpen={isAddingEdu} onClose={() => { setIsAddingEdu(false); setEditingEduId(null); setEduForm({ institution: '', degree: '', field_of_study: '', start_date: '', end_date: '' }); }} title={editingEduId ? 'Edit Education Route' : 'Add Education Route'}>
                <form onSubmit={handleSaveEdu} style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
            ...
            <div style={{ display: activeTab === 'skills' ? 'block' : 'none', width: '100%' }}>
              ...
              <Modal isOpen={isAddingSkill} onClose={() => { setIsAddingSkill(false); setEditingSkillId(null); setSkillForm({ skill_name: '', category: '' }); }} title={editingSkillId ? 'Edit Skill' : 'Batch Add Technical Skills'}>
                <form onSubmit={handleSaveSkill} style={{ marginBottom: '20px' }}>
